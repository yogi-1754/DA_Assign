...
/*1.	Fetch the employee number, first name and last name of those employees who are working as Sales Rep reporting
 to employee with  employeenumber 1102*/
...
/*1.	Using the OrderDetails table, identify the top 10 products (by productCode) with the highest total order
...
/*2.	Company wants to analyze payment frequency by month. Extract the month name from the payment date to
 count the total number of payments for
...
-- Use the Customers_Orders database
...
SUM(ORDERDETAILS.QUANTITYORDERED * ORDERDETAILS.PRICEEACH) AS TOTAL_SALES,
 COUNT(DISTINCT ORDERS.ORDERNUMBER) AS NO_OF_ORDERS
FROM PRODUCTS
JOIN 
...
1.	Create a stored procedure Get_country_payments which takes in year and country as inputs and gives year wise,
 country wise total amount as an output. Format the total amount to nearest thousand unit (K)
Tables: Customers, Payments*/

call classicmodels.GET_COUNTRY_PAYMENTS(2003, 'FRANCE', @TOTALAMOUNT);
select @TOTALAMOUNT;
...
select customername,count(orders.ordernumber) as order_count,
rank() over(order by count(orders.ordernumber)desc) as order_frequency_rank
...
/*
1.	Find out how many product lines are there for which the buy price value is greater than the  average of MSRP value.
...
call emp_eh(1,'amit','a@gmail.com');
...
SELECT * FROM EMP_BIT;
insert into emp_bit value ('rohit','cricketer','2020-10-04',-16);
